...
void Control::filepathHandler(string &filepath)
{
	...
	int delimeterIndex;
	delimeterIndex = (isFile == true) ? filepath.find(".") :
					    filepath.length() - 1;
...
void Control::traverseFile()
{
	...
	while (std::getline(this->readFile, currentCommand)) {
		...
		currentCommand.erase(
			currentCommand.find_last_not_of("\t\n\v\f\r ") +
			1); // right trim
		if (currentCommand.substr(0, 2) != "//" &&
			...
			case C_PUSH:
				...
				if (this->parser.getArg1() == "constant") {
					...
					valueToPush =
						this->memoryManager.getMSLocal(
							this->parser.getArg2());
				} else if (this->parser.getArg1() ==
					   "argument") {
					valueToPush =
						this->memoryManager.getMSArgument(
							this->parser.getArg2());
				} else if (this->parser.getArg1() == "this") {
					valueToPush =
						this->memoryManager.getMSThis(
							this->parser.getArg2());
				} else if (this->parser.getArg1() == "that") {
					valueToPush =
						this->memoryManager.getMSThat(
							this->parser.getArg2());
				} else if (this->parser.getArg1() == "static") {
					valueToPush =
						this->memoryManager.getMSStatic(
							this->parser.getArg2());
				} else if (this->parser.getArg1() ==
					   "pointer") {
					valueToPush =
						this->memoryManager.getMSPointer(
							this->parser.getArg2());
				} else if (this->parser.getArg1() == "temp") {
					valueToPush =
						this->memoryManager.getMSTemp(
							this->parser.getArg2());
				} else {
					valueToPush = 0;
					std::cout << ".vm file incorrect"
						  << std::endl;
					exit(0);
				}
				this->memoryManager.updateStackMemory(
			...
			case C_POP:
				...
				if (this->parser.getArg1() == "local") {
					...
					this->memoryManager.updateMSThis(
						this->parser.getArg2(),
						resultPop);
				} else if (this->parser.getArg1() == "that") {
					this->memoryManager.updateMSThat(
						this->parser.getArg2(),
						resultPop);
				} else if (this->parser.getArg1() == "static") {
					...
					this->memoryManager.updateMSTemp(
						this->parser.getArg2(),
						resultPop);
				} else {
					std::cout << ".vm file incorrect"
