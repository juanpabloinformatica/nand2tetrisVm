...
Parser::~Parser () {
    cout << "Removing parser" << endl;
}

void Parser::operateCommand (string currentCommand) {
    this->setSplitCurrentCommand (currentCommand);
    this->setCommandType (this->getMapCommand ().at (this->getSplitCurrentCommand ()[0]));
    ...
    if (this->getCommandType () != C_RETURN) {
        ...
        this->getCommandType () == C_FUNCTION || this->getCommandType () == C_CALL) {
            this->setArg2 (stoi (this->getSplitCurrentCommand ()[2]));
        }
    }
    this->resetSplitCurrentCommand ();
}
void Parser::setArg1 (string arg1) {
    this->arg1 = arg1;
}
void Parser::setArg2 (int arg2) {
    this->arg2 = arg2;
}
...
COMMAND_TYPE& Parser::getCommandType () {
